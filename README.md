...
It is deliberately narrow: vocabulary-channel QA, **not** a model-safety scanner or backdoor detector. A flag is a candidate to investigate, not a verdict; a clean run is not a certificate.
...
```mermaid
flowchart TD
  T["you suspect a token τ (the decisive path)"] --> C{"classify --token τ: does its elevation collapse under orthogonalization? (drift-immune)"}
...
|---|---|---|---|
| **Carrier classifier** (the decisive, drift-immune probe) | runs the remover as a **probe** — orthogonalize the flagged subspace, check whether the elevation collapses. Collapse ⇒ vocabulary-carried & fixable; no collapse ⇒ **escalate**. This is the headline capability: targeted at a token *you* name, it is not confounded by benign drift | **E2** (`evidence/E2_RESULT.md`) | validated on a token-trait vs a sycophancy student: token-trait → "fixable" (handle = " seven", residual 0); sycophancy (interaction +7.07) → ablating the top flag does not reduce the interaction (+7.07 → +8.76) → "escalate" (no single-token handle). A quiet/diffuse scan on a conditional policy is **not** an all-clear; single-family (Gemma-3-1b), matching the paper's §6.4 scope |
...
*What `scrub` does, and why it isn't a lobotomy: orthogonalizing W_τ against its entangled neighbours collapses the masked leak to ≈0 while overt transfer and perplexity are preserved; a same-rank random placebo leaves it intact — the specificity control.*
